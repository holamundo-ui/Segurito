...
async function envioCorreo(mailOptions, intentos = 3) {
    try {
        ...
        if (intentos > 1) {
            ...
            setTimeout(() => envioCorreo(mailOptions, intentos - 1), 5000);
        } else {
...
async function enviarCorreoDesbloqueo(usuario) {
...
async function enviarCorreoUsuarioNoRegistrado(usuario) {
    const correoUsuario = obtenerCorreoUsuario(usuario);
    if (!correoUsuario) {
        console.error(`Correo no encontrado para el usuario ${usuario}`);
        return;
...
async function enviarCorreoUsuarioNoBloqueado(usuario) {
    const correoUsuario = obtenerCorreoUsuario(usuario);
    if (!correoUsuario) {
        console.error(`Correo no encontrado para el usuario ${usuario}`);
        return;
...
module.exports = {
    ...
    enviarCorreoUsuarioNoBloqueado // Nueva función agregada
};
